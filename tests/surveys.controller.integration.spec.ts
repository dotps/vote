...
import { Question } from "../src/surveys/question.entity"
...
import { UpdateAnswerDto } from "../src/surveys/update-survey.dto"
...
describe("SurveysController (интеграционный): ", () => {
    ...
    beforeEach(async () => {
        ...
        // Обновляем mockSaveSurveyResultDto после создания опроса
        if (createdSurveyId) {
            const surveyResponse = await request(app.getHttpServer())
                .get(`/surveys/${createdSurveyId}`)
                .set("Authorization", `Bearer ${authToken}`)
    ...
    describe("получение опросов: ", () => {
        ...
        it("получить результаты опроса", async () => {
            // Сначала сохраняем результат опроса
